...
function handleCardClick(event) {
  ...
  let color = event.target.classList[event.target.classList.length-1];
  ...
  if(firstColor !== "" && secondColor !== "" && firstColor === secondColor) {
    let tiles = document.querySelectorAll(`.${firstColor}`);
  ...
  if(firstColor !== "" && secondColor !== "" && firstColor !== secondColor) {
    ...
    tiles = document.querySelectorAll(`.${secondColor}`);
    for(let tile of tiles) {
      tile.style = "background-color: white;";
    }
    firstColor = "";
